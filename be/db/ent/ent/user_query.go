...
func (uq *UserQuery) QueryArticles() *ArticleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, true, user.ArticlesTable, user.ArticlesColumn),
		)
...
func (uq *UserQuery) loadArticles(ctx context.Context, query *ArticleQuery, nodes []*User, init func(*User), assign func(*User, *Article)) error {
	...
	for _, n := range neighbors {
		fk := n.article_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "article_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "article_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
